import "clsx";
import { H as Header } from "../../../chunks/Header.js";
import { x as await_block, y as ensure_array_like } from "../../../chunks/index.js";
import { GraphQLClient, gql } from "graphql-request";
import { k as escape_html } from "../../../chunks/context.js";
function Gegevens($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    const getData = async () => {
      const endpoint = `https://api-eu-central-1-shared-euc1-02.hygraph.com/v2/clilodskp031201uhbhpdaltk/master`;
      const graphQLClient = new GraphQLClient(endpoint, {
        headers: {
          authorization: `Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6ImdjbXMtbWFpbi1wcm9kdWN0aW9uIn0..mtX12JDjdjYcfHqwUg8HeDHMSvfRi2KVhCxRU5r4chMmpQX1BvgZgDvml03NI7zsMZKeVWL_9YIDrjJHw12M-uwWbAbERQlZMhENVIduCyOVfVkZrd31t1tPMikqo2jH3jtMgL-1H01Svk4SdlSub04BTzdYs7s4Xs0z-BCA5EGRr9fXWg75_ckhla4nyYsF0i4v2SazPhgZuD-kjVSK6mlmMCr0kbQg_3vSwNwlS2FWcBTrYR4NXKdWgM2kxipdve74chk8S6o-U_SqvatZUHIAG264asJm67r0q83sPG3sqRAjaC2QyTV8-6dhxSJKI65mfpoxXRORBfLk5wWfAqK8CRAe0p0RyznS_kiPpUyAaOT5hMLwufKKRYBuqgAN7DtK8WWU8uSVFLHHZeK2SM-uGTi3oFM_aRP9pOGVX5pjJnjII6yxedWa3jsb52nJ3neUo_oXIv7LbNfaxPTg6qGICp7jQJ-TQRQ1IDTGpQGYu0BXftXsqZKYTVYDS0Swme1ZMlOgkf-nX2wZM85ktdugZL7Kna1yWbZpOMJx_pnHhAj-yzh1Id7qrnYrHGL220cXVBRKNQJz0A0VXNWOtMG5P7DyuEAd9zhoOA4YgOcxyXnrFls9a8qL-x0ADqWf2HbndxA7evk3ByGMvQz9NwhHscp2aCC0vjvtfBQ2OCM`
        }
      });
      const query = gql`
    query {
      gegevens(stage: DRAFT, last: 1) {
        aantalRegenpijpen
        dakOppervlakte
        inhoudRegenton
        typeDak
        postcode
        huisnummer
      }
    }
    `;
      const data = await graphQLClient.request(query);
      console.log(data.gegevens);
      return data;
    };
    await_block($$renderer2, getData(), () => {
    }, (data) => {
      $$renderer2.push(`<!--[-->`);
      const each_array = ensure_array_like(data.gegevens);
      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
        let gegevens = each_array[$$index];
        $$renderer2.push(`<section class="gegevens svelte-1g6jt66"><h2 class="svelte-1g6jt66">Gegevens</h2> <section class="svelte-1g6jt66"><section class="svelte-1g6jt66"><p class="svelte-1g6jt66">Daktype</p> <p class="svelte-1g6jt66">${escape_html(gegevens.typeDak)}</p></section> <div class="svelte-1g6jt66"></div></section> <section class="svelte-1g6jt66"><section class="svelte-1g6jt66"><p class="svelte-1g6jt66">Aantal vierkante meter van het huis</p> <p class="svelte-1g6jt66">${escape_html(gegevens.dakOppervlakte)}</p></section> <div class="svelte-1g6jt66"></div></section> <section class="svelte-1g6jt66"><section class="svelte-1g6jt66"><p class="svelte-1g6jt66">Aantal regenpijpen</p> <p class="svelte-1g6jt66">${escape_html(gegevens.aantalRegenpijpen)}</p></section> <div class="svelte-1g6jt66"></div></section> <section class="svelte-1g6jt66"><section class="svelte-1g6jt66"><p class="svelte-1g6jt66">Inhoud regenton</p> <p class="svelte-1g6jt66">${escape_html(gegevens.inhoudRegenton)}L</p></section> <div class="svelte-1g6jt66"></div></section> <section class="svelte-1g6jt66"><section class="svelte-1g6jt66"><p class="svelte-1g6jt66">Adres</p> <p class="svelte-1g6jt66">${escape_html(gegevens.postcode)}, ${escape_html(gegevens.huisnummer)}</p></section> <div class="svelte-1g6jt66"></div></section></section>`);
      }
      $$renderer2.push(`<!--]-->`);
    });
    $$renderer2.push(`<!--]-->`);
  });
}
function _page($$renderer) {
  Header($$renderer);
  $$renderer.push(`<!----> <main class="check svelte-1258qcz">`);
  Gegevens($$renderer);
  $$renderer.push(`<!----> <a href="/home">Opslaan</a></main>`);
}
export {
  _page as default
};
